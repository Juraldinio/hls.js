...
class StreamLoader {
  ...
  load (context, config, callbacks) {
    let stats = this.stats = { trequest: performance.now(), retry: 0, loaded: 0, chunks: 0 }, headers = {};
    ...
    this.req = http.get({
      ...
      res.on('end', function () {
        ...
        callbacks.onSuccess({ url: context.url }, stats, context);
      });
      ...
      res.on('error', function (err) {
        callbacks.onError({ text: err.message }, context);
      });
